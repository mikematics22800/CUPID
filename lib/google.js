...
// Function to search venues using Google Places Autocomplete API
export const searchVenues = async (query, types = ['establishment']) => {
  try {
    if (!query.trim() || query.length < 1) {
      return [];
    }
    ...
    // Use the Places Autocomplete API for venue searches
    const response = await fetch(
      `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(query)}&types=${types.join('|')}&key=${key}`
    );
    
    const data = await response.json();
    
    if (data.status === 'OK' && data.predictions) {
      const venuePredictions = data.predictions
        .filter(prediction => 
          prediction.types && (
            ...
            prediction.types.includes('park') ||
            prediction.types.includes('movie_theater') ||
            prediction.types.includes('amusement_park') ||
            prediction.types.includes('museum') ||
            prediction.types.includes('art_gallery') ||
            prediction.types.includes('night_club') ||
      ...
      console.log(`✅ Found ${venuePredictions.length} venue predictions`);
      return venuePredictions;
    } else {
      console.error('❌ Places Autocomplete API error:', data.status, data.error_message);
      console.log('Response data:', JSON.stringify(data, null, 2));
      return [];
...
// Function to get venue details using Google Places API
export const getVenueDetails = async (placeId) => {
  try {
    ...
    const response = await fetch(
      `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&fields=name,formatted_address,geometry,types,rating,opening_hours,price_level,photos&key=${key}`
    );
    ...
    if (data.status === 'OK' && data.result) {
      ...
      console.log(`✅ Retrieved venue details: ${venue.name} - ${venue.formatted_address}`);
      ...
      console.error('❌ Venue details error:', data.status, data.error_message);
      console.log('Response data:', JSON.stringify(data, null, 2));
      return null;
    }
  } catch (error) {
...
// Function to search venues by category (restaurants, parks, etc.)
export const searchVenuesByCategory = async (query, category = 'restaurant') => {
  try {
    if (!query.trim() || query.length < 1) {
      return [];
    }
    ...
    console.log(`🏢 Searching ${category} venues for: ${query}`);
    ...
    const categoryTypes = {
      ...
      'entertainment': ['amusement_park', 'aquarium', 'zoo'],
      'all': ['establishment', 'point_of_interest']
    };
    
    const types = categoryTypes[category] || categoryTypes['all'];
    
    return await searchVenues(query, types);
  } catch (error) {
