...
export const sendMessage = async (roomId, content) => {
  try {
    ...
    const { data, error } = await supabase
      .from('messages')
      .insert({
        id: currentUser.id, // Using current user's ID as message ID
    ...
    const formattedMessage = {
      ...
      sender_id: data.id, // Using the message id as sender_id
...
export const getMessages = async (roomId, limit = 50, offset = 0) => {
  try {
    ...
    const { data, error } = await supabase
      .from('messages')
      .select('*')
      .or(`and(id.eq.${currentUser.id},receiver_id.eq.${otherUserId}),and(id.eq.${otherUserId},receiver_id.eq.${currentUser.id})`)
      .order('created_at', { ascending: false })
    ...
    const senderIds = [...new Set(data.map(message => message.id))];
    ...
    const processedMessages = data.map(message => ({
      ...
      sender_id: message.id, // The id field represents the sender
      sender_name: userMap[message.id]?.name || 'Unknown User'
    }));
    ...
    const processedRooms = matches.map(match => {
      ...
      return {
        ...
        lastMessage: lastMessage ? {
          ...
          isFromMe: lastMessage.id === currentUser.id // The id field represents the sender
        } : null,
        ...
        const subscription = supabase
          ...
          .on(
            'postgres_changes',
            {
              ...
              schema: 'public',
              table: 'messages',
              filter: `or(and(id.eq.${currentUser.id},receiver_id.eq.${otherUserId}),and(id.eq.${otherUserId},receiver_id.eq.${currentUser.id}))`
            },
            (payload) => {
...
export const deleteMessage = async (messageId) => {
  try {
    ...
    const { data, error } = await supabase
      .from('messages')
      .delete()
      .eq('id', currentUser.id) // The id field represents the sender
      .eq('receiver_id', messageId); // messageId here represents the receiver
...
export const deleteUserAccount = async () => {
  try {
    ...
    try {
      ...
      if (!matchesError && matches && matches.length > 0) {
        ...
        const { error: messagesError } = await supabase
          .from('messages')
          .delete()
          .or(`id.eq.${userId},receiver_id.eq.${userId}`);
    ...
    // 5. Delete quiz records for the user
    try {
      ...
      if (quizError) {
        ...
        console.log('✅ User quiz records deleted');
      }
    } catch (quizError) {
      console.error('❌ Error handling quiz deletion:', quizError);
    }
...
export const createOrUpdateQuiz = async (userId, quizData) => {
  try {
    ...
    // Update the user's quiz field directly in the users table
    const { data, error } = await supabase
      .from('users')
      .update({ 
        quiz: quizData
