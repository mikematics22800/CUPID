...
// Helper function to check if a match exists between two users
const checkMatchExists = async (user1Id, user2Id) => {
  try {
    // Try first combination: user1Id as user1, user2Id as user2
...
export const saveQuizScore = async (quizOwnerId, quizTakerId, score) => {
  try {
    ...
    let match = await checkMatchExists(quizTakerId, quizOwnerId);

    if (!match) {
      console.log(`📝 No match found, creating new match between ${quizTakerId} and ${quizOwnerId}`);
      
      // Create a new match if one doesn't exist
      const { data: newMatch, error: createError } = await supabase
        .from('matches')
        .insert({
          user1_id: quizTakerId,
          user2_id: quizOwnerId
        })
        .select('*')
        .single();
    ...
    if (match.user1_id === quizTakerId) {
      ...
      console.log(`📝 Updating user1_score to ${score}%`);
    } else {
      // Quiz taker is user2, quiz owner is user1
      updateData.user2_score = score;
      console.log(`📝 Updating user2_score to ${score}%`);
    }
...
export const getQuizScoreForUser = async (quizOwnerId) => {
  try {
    ...
    const { data: matches, error: matchError } = await supabase
      ...
      .or(`user1_id.eq.${currentUser.id},user2_id.eq.${quizOwnerId}`)
      .or(`user1_id.eq.${quizOwnerId},user2_id.eq.${currentUser.id}`);
