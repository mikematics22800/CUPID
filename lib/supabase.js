...
// Enhanced error logging utility - DEPRECATED: Use console.error directly
export const logSupabaseError = (operation, error, additionalData = {}) => {
  console.error(`🔴 Supabase Error in ${operation}:`, error);
  return { operation, error };
};

// Enhanced success logging utility - DEPRECATED: Use console.log directly
export const logSupabaseSuccess = (operation, data, additionalData = {}) => {
  console.log(`🟢 Supabase Success in ${operation}`);
  return { operation, data };
};

// Wrapper function for database operations with enhanced logging - DEPRECATED
export const executeWithLogging = async (operation, supabaseCall) => {
  console.warn('⚠️ executeWithLogging is deprecated. Use direct supabase calls.');
...
export const createMatch = async (user1Id, user2Id) => {
  try {
    ...
    // Try first combination: user1Id as user1, user2Id as user2
    let { data: existingMatch, error: findError } = await supabase
      .from('matches')
      .select('*')
      .eq('user1_id', user1Id)
      .eq('user2_id', user2Id)
    ...
    // If not found, try the reverse combination
    if (findError && findError.code === 'PGRST116') {
      ...
      if (reverseError && reverseError.code === 'PGRST116') {
        // No match exists, continue to create one
        existingMatch = null;
      } else if (reverseError) {
        console.error('❌ Error checking for reverse match:', reverseError);
        throw reverseError;
      } else {
        existingMatch = reverseMatch;
      }
    } else if (findError) {
...
export const getSwipeProfiles = async (limit = 10) => {
  try {
    ...
    let query = supabase
      ...
      .eq('banned', false) // Exclude banned users
...
export const getMatchesForUser = async () => {
  try {
    ...
    const { data: matches, error: matchesError } = await supabase
      .from('matches')
      .select('*, user1:users!matches_user1_id_fkey(id, name, bio, birthday, sex, interests, images, residence, geolocation, banned), user2:users!matches_user2_id_fkey(id, name, bio, birthday, sex, interests, images, residence, geolocation, banned)')
    ...
    const processedMatches = await Promise.all(
      matches.map(async (match) => {
        ...
        const userAttempts = isUser1 ? match.user1_attempts : match.user2_attempts;
        const otherUserAttempts = isUser1 ? match.user2_attempts : match.user1_attempts;
        ...
        // Skip banned users
        if (otherUser.banned) {
          console.log(`🚫 Skipping banned user ${otherUser.id} from matches`);
          return null;
        }
        ...
        return {
          ...
          otherUserAttempts: otherUserAttempts,
...
export const unmatchUsers = async (user1Id, user2Id) => {
  try {
    console.log(`❌ Unmatching users ${user1Id} and ${user2Id} (using deprecated function)`);

    // First, find the match to get its ID
    ...
    return await unmatchUsersByMatchId(existingMatch.id);
...
export const unmatchUsersByMatchId = async (matchId) => {
  try {
    ...
    // First, get the match details to find the users involved
    ...
    // Delete all messages between these users first
    try {
      // First, get all message IDs between these users
      ...
      if (fetchError) {
      ...
      } else if (messagesToDelete && messagesToDelete.length > 0) {
        console.log(`🗑️ Found ${messagesToDelete.length} messages to delete between users ${user1_id} and ${user2_id}`);
        ...
        if (messagesDeleteError) {
          ...
          console.log(`✅ Deleted ${messageIds.length} messages between users ${user1_id} and ${user2_id}`);
        }
      } else {
        console.log(`📭 No messages found between users ${user1_id} and ${user2_id}`);
      }
    } catch (messagesError) {
    ...
    // Delete the match record using the match ID
    ...
    console.log(`✅ Successfully unmatched using match ID: ${matchId}`);
    return true;
          ...
          const subscription = supabase
            ...
            .on(
              ...
              (payload) => {
                ...
                if ((message.sender_id === currentUser.id && message.receiver_id === otherUserId) ||
                    (message.sender_id === otherUserId && message.receiver_id === currentUser.id)) {
...
export const deleteMessage = async (messageId) => {
  try {
    ...
    if (fetchError) {
      ...
      // Let's try to list all messages to see what's in the database
      ...
      if (!listError) {
        console.log('🗑️ Available messages in database:', allMessages);
      }
    ...
    console.log(`🗑️ Message sender_id: ${message.sender_id}`);
    console.log(`🗑️ Current user ID: ${currentUser.id}`);
    console.log(`🗑️ IDs match: ${message.sender_id === currentUser.id}`);

    // Check if the sender_id is a valid user ID (not a random UUID)
    if (message.sender_id !== currentUser.id) {
      console.error('❌ User not authorized to delete this message');
      console.error('❌ This might be due to database schema issues with sender_id defaults');
...
// Function to check and fix database schema issues - DEPRECATED: No longer needed
export const checkAndFixMessageSchema = async () => {
  console.warn('⚠️ checkAndFixMessageSchema is deprecated and no longer needed.');
  return {
    totalMessages: 0,
    userMessages: 0,
    messages: []
  ...
  try {
    ...
    try {
      ...
      if (!matchesError && matches && matches.length > 0) {
        ...
        // First, get all message IDs where the user is either sender or receiver
        const { data: messagesToDelete, error: fetchError } = await supabase
          .from('messages')
          .select('id')
        ...
        if (fetchError) {
          ...
          console.log(`🗑️ Found ${messagesToDelete.length} messages to delete for user ${userId}`);
    ...
    // 5. Quiz data is stored in the quizzes table, so it will be deleted when the user profile is deleted
...
export const getUsersWithinDistance = async (maxDistance = 50, limit = 50) => {
  try {
    ...
    // Get all user profiles (excluding current user and banned users)
...
export const createOrUpdateQuiz = async (userId, quizData) => {
  try {
    ...
    const fakeAnswers = quizData.map(item => [item[2], item[3], item[4]]); // Get all 3 fake answers
...
export const submitQuizAnswers = async (quizOwnerId, answers) => {
  try {
    ...
    // Get the quiz owner's quiz with the same structure as getQuizWithOptions
    const { data: quiz, error } = await supabase
      .from('quizzes')
      .select('questions, answers, fake_answers')
      .eq('id', quizOwnerId);
    ...
    // Use the first quiz found (there should only be one per user)
    ...
    for (let i = 0; i < totalQuestions; i++) {
      if (answers[i] && quizData.answers[i] && answers[i] === quizData.answers[i]) {
        correctAnswers++;
      }
    }
...
export const saveQuizScore = async (quizOwnerId, quizTakerId, score) => {
  try {
    ...
    // Find the existing match between the two users
    console.log(`🔍 Searching for match between quizTakerId: ${quizTakerId} and quizOwnerId: ${quizOwnerId}`);
    
    // Query for the specific match between these two users
    // First try: quizTaker as user1, quizOwner as user2
    let { data: match, error: matchError } = await supabase
      .from('matches')
      .select('*')
      .eq('user1_id', quizTakerId)
      .eq('user2_id', quizOwnerId)
      .single();

    // If not found, try the reverse: quizOwner as user1, quizTaker as user2
    if (matchError && matchError.code === 'PGRST116') {
      console.log(`🔍 First query failed, trying reverse order...`);
      const { data: reverseMatch, error: reverseError } = await supabase
        .from('matches')
        .select('*')
        .eq('user1_id', quizOwnerId)
        .eq('user2_id', quizTakerId)
        .single();
      
      if (reverseError) {
        console.error('❌ Error finding match (both directions failed):', reverseError);
        throw reverseError;
      }
      match = reverseMatch;
      matchError = null;
    ...
    console.log(`🔍 Match query results:`, match);
    ...
    // Determine which user is the current logged-in user (quiz taker) and update the appropriate score and attempts
    let updateData = {};
    if (match.user1_id === quizTakerId) {
      // Current user (quiz taker) is user1, quiz owner is user2
      const currentScore = match.user1_score || 0;
      const currentAttempts = match.user1_attempts || 0;
      
      // Only update score and increment attempts if the new score is greater than the previous high score
      const shouldUpdateScore = numericScore > currentScore;
      const newAttempts = shouldUpdateScore ? currentAttempts + 1 : currentAttempts;
      
      if (shouldUpdateScore) {
        updateData.user1_score = numericScore;
        updateData.user1_attempts = newAttempts;
        console.log(`📝 Updating user1_score to ${numericScore}% and user1_attempts to ${newAttempts} (current user ${quizTakerId} is user1, previous score: ${currentScore}%, new high score!)`);
      } else {
        console.log(`📝 Score ${numericScore}% is not higher than previous score ${currentScore}% for user1 (${quizTakerId}), no update needed`);
      }
    } else if (match.user2_id === quizTakerId) {
      // Current user (quiz taker) is user2, quiz owner is user1
      const currentScore = match.user2_score || 0;
      const currentAttempts = match.user2_attempts || 0;
      ...
      if (shouldUpdateScore) {
        updateData.user2_score = numericScore;
        updateData.user2_attempts = newAttempts;
        console.log(`📝 Updating user2_score to ${numericScore}% and user2_attempts to ${newAttempts} (current user ${quizTakerId} is user2, previous score: ${currentScore}%, new high score!)`);
      } else {
        console.log(`📝 Score ${numericScore}% is not higher than previous score ${currentScore}% for user2 (${quizTakerId}), no update needed`);
      }
    } else {
      console.error('❌ Error: Current user ID does not match either user in the match');
      throw new Error('Current user ID does not match either user in the match');
    }
    ...
    console.log(`✅ Quiz score saved successfully: ${numericScore}%`);
  } catch (error) {
...
export const getQuizScoreForUser = async (quizOwnerId) => {
  try {
    ...
    // Find the match between current user and quiz owner using the same pattern as checkMatchExists
    const { data: matches, error: matchError } = await supabase
      .from('matches')
      .select('user1_id, user2_id, user1_score, user2_score')
    ...
    // Return the first match found (there should only be one)
...
export const getQuizScoreForUserReversed = async (otherUserId) => {
  try {
    ...
    // Return the other user's score (opposite of current user's score)
    if (match.user1_id === currentUser.id) {
      return match.user2_score;
    } else {
      return match.user1_score;
