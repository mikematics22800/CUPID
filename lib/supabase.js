...
export const handleUserLike = async (likedUserId) => {
  try {
    ...
    // Step 1: Add current user to liked user's likes array
    ...
    if (fetchError && fetchError.code !== 'PGRST116') {
      console.error('❌ Error fetching liked user record:', fetchError);
      throw fetchError;
    }
    ...
    if (existingLikeRecord && existingLikeRecord.likes) {
      if (typeof existingLikeRecord.likes === 'string') {
        try {
          likedUserLikes = JSON.parse(existingLikeRecord.likes);
        } catch (parseError) {
          console.error('❌ Failed to parse likes as JSON:', parseError);
          likedUserLikes = [];
        }
      } else if (Array.isArray(existingLikeRecord.likes)) {
        likedUserLikes = existingLikeRecord.likes;
      }
    }

    console.log(`📊 Liked user's likes array:`, likedUserLikes);

    // Check if current user is already in the liked user's likes array
    if (likedUserLikes.includes(currentUser.id)) {
      console.log('⚠️ Current user already in liked user\'s likes array');
    } else {
      // Add current user's ID to the liked user's likes array
      const updatedLikedUserLikes = [...likedUserLikes, currentUser.id];
      console.log(`📝 Updated liked user's likes array:`, updatedLikedUserLikes);
      ...
      if (existingLikeRecord) {
        ...
        updateResult = await supabase
          .from('likes')
          .update({ likes: updatedLikedUserLikes })
          .eq('id', likedUserId)
          .select('likes')
          .single();
      } else {
      ...
      if (updateResult.error) {
        console.error('❌ Error updating liked user\'s likes:', updateResult.error);
        throw updateResult.error;
      }

      console.log(`✅ Successfully added current user to liked user's likes array`);
    }

    // Step 2: Ensure current user has the liked user in their own likes array
    const { data: currentUserLikesRecord, error: currentUserLikesError } = await supabase
      .from('likes')
      .select('likes')
    ...
    // Add liked user to current user's likes if not already there
    if (!currentUserLikes.includes(likedUserId)) {
      currentUserLikes.push(likedUserId);
      
      let currentUserUpdateResult;
      if (currentUserLikesRecord) {
        currentUserUpdateResult = await supabase
          .from('likes')
          .update({ likes: currentUserLikes })
          .eq('id', currentUser.id)
          .select('likes')
          .single();
      } else {
      ...
      if (currentUserUpdateResult.error) {
        console.error('❌ Error updating current user likes:', currentUserUpdateResult.error);
        throw currentUserUpdateResult.error;
      }
      
      console.log(`✅ Added liked user to current user's likes array`);
    } else {
      console.log(`ℹ️ Liked user already in current user's likes array`);
    }
    
    // Step 3: Check if this creates a mutual match
...
export const getUsersWhoLikedMe = async () => {
  try {
    ...
    // Get current user's discarded likes
    const { data: discardedRecord, error: discardedError } = await supabase
      .from('discarded_likes')
      .select('discarded_likes')
      .eq('id', currentUser.id)
      .single();

    let discardedUserIds = [];
    if (discardedRecord && discardedRecord.discarded_likes) {
      if (typeof discardedRecord.discarded_likes === 'string') {
        try {
          discardedUserIds = JSON.parse(discardedRecord.discarded_likes);
        } catch (parseError) {
          console.error('❌ Failed to parse discarded likes as JSON:', parseError);
          discardedUserIds = [];
        }
      } else if (Array.isArray(discardedRecord.discarded_likes)) {
        discardedUserIds = discardedRecord.discarded_likes;
      }
    }
    ...
    for (const record of allLikesRecords) {
      if (record.id === currentUser.id) continue; // Skip current user's own record
      ...
      if (likesArray.includes(currentUser.id)) {
        // Check if this user has been discarded
        if (!discardedUserIds.includes(record.id)) {
          usersWhoLikedMe.push(record.id);
        }
      }
    }

    console.log(`📊 Found ${usersWhoLikedMe.length} users who liked current user (after filtering discarded)`);
...
// Function to discard a like (add to discarded likes)
export const discardLike = async (discardedUserId) => {
  try {
    ...
    // Check if the current user already has a discarded likes record
    const { data: existingDiscardedRecord, error: fetchError } = await supabase
      .from('discarded_likes')
      .select('discarded_likes')
      .eq('id', currentUser.id)
      .single();
    ...
    if (fetchError && fetchError.code !== 'PGRST116') {
      console.error('❌ Error fetching discarded likes record:', fetchError);
      throw fetchError;
    }

    let currentDiscardedLikes = [];
    if (existingDiscardedRecord && existingDiscardedRecord.discarded_likes) {
      if (typeof existingDiscardedRecord.discarded_likes === 'string') {
        try {
          currentDiscardedLikes = JSON.parse(existingDiscardedRecord.discarded_likes);
        } catch (parseError) {
          console.error('❌ Failed to parse discarded likes as JSON:', parseError);
          currentDiscardedLikes = [];
        }
      } else if (Array.isArray(existingDiscardedRecord.discarded_likes)) {
        currentDiscardedLikes = existingDiscardedRecord.discarded_likes;
      }
    }

    // Check if user is already in discarded list
    if (currentDiscardedLikes.includes(discardedUserId)) {
      console.log('⚠️ User already discarded this like');
      return { success: true, alreadyDiscarded: true };
    }

    // Add discarded user's ID to the discarded likes array
    const updatedDiscardedLikes = [...currentDiscardedLikes, discardedUserId];
    console.log(`📝 Updated discarded likes array:`, updatedDiscardedLikes);

    let updateResult;
    if (existingDiscardedRecord) {
      // Update existing record
      updateResult = await supabase
        .from('discarded_likes')
        .update({ discarded_likes: updatedDiscardedLikes })
        .eq('id', currentUser.id)
        .select('discarded_likes')
        .single();
    } else {
      ...
      updateResult = await supabase
        ...
        .insert({
          id: currentUser.id,
          discarded_likes: updatedDiscardedLikes
        })
        .select('discarded_likes')
        .single();
    }
    ...
    if (updateResult.error) {
      console.error('❌ Error updating discarded likes:', updateResult.error);
      throw updateResult.error;
    }

    console.log(`✅ Successfully updated discarded likes`);
