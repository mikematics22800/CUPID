...
export const unmatchUsers = async (user1Id, user2Id) => {
  try {
    ...
    // First, find the match to get its ID before deleting it
    ...
    if (existingMatch) {
      try {
        ...
        if (fetchError) {
          ...
          console.log(`🗑️ Found ${messagesToDelete.length} messages to delete between users ${user1Id} and ${user2Id}`);
          ...
          if (messagesDeleteError) {
            ...
            console.log(`✅ Deleted ${messageIds.length} messages between users ${user1Id} and ${user2Id}`);
          }
        } else {
          console.log(`📭 No messages found between users ${user1Id} and ${user2Id}`);
        }
      } catch (messagesError) {
...
// Function to check and fix database schema issues
export const checkAndFixMessageSchema = async () => {
  try {
    ...
    // Check if there are any messages with invalid sender_id or receiver_id
    const { data: invalidMessages, error: checkError } = await supabase
      .from('messages')
    ...
    console.log('🔧 Found messages:', invalidMessages);
    
    // Check if any messages have sender_id that doesn't match current user
    const userMessages = invalidMessages?.filter(msg => msg.sender_id === currentUser.id) || [];
    console.log('🔧 User messages:', userMessages);
    
    return {
      totalMessages: invalidMessages?.length || 0,
      userMessages: userMessages.length,
      messages: invalidMessages
    };
  } catch (error) {
    console.error('❌ Error in checkAndFixMessageSchema:', error);
    throw error;
  }
};
...
export const createOrUpdateQuiz = async (usesirId, quizData) => {
  try {
...
export const saveQuizScore = async (quizOwnerId, quizTakerId, score) => {
  try {
    ...
    if (match.user1_id === quizTakerId) {
      ...
      updateData.user1_attempts = Math.max(0, (match.user1_attempts || 0)) + 1;
      console.log(`📝 Updating user1_score to ${numericScore}% and user1_attempts to ${updateData.user1_attempts} (current user ${quizTakerId} is user1)`);
      ...
      updateData.user2_attempts = Math.max(0, (match.user2_attempts || 0)) + 1;
      console.log(`📝 Updating user2_score to ${numericScore}% and user2_attempts to ${updateData.user2_attempts} (current user ${quizTakerId} is user2)`);
    } else {
