  ...
  try {
    ...
    const processedLikes = await Promise.all(
      likesReceived.map(async (like) => {
        ...
        if (!likerUser) {
          console.warn(`⚠️ Missing user profile for liker ${like.id}`);
          return null;
        }
...
export const deleteUserAccount = async () => {
  try {
    ...
    console.log('✅ Quiz data will be deleted from quiz table');
...
export const generateQuiz = async (userId, numQuestions = 10) => {
  try {
    console.log(`🤖 Generating quiz for user: ${userId} with ${numQuestions} questions`);
    ...
    if (profileError) {
      console.error('❌ Error fetching user profile for quiz generation:', profileError);
...
- Generate exactly ${numQuestions} questions`;
    ...
    console.log(`✅ Quiz generated and saved successfully for user: ${userId}`);
    return {
      questions: quizData,
      data: data
    };
...
export const getUserQuiz = async (userId) => {
  try {
    ...
    try {
      ...
      console.warn('⚠️ Error parsing fake_answers, using empty array:', parseError);
      fakeAnswers = [];
    }
    ...
    console.log(`✅ Retrieved quiz for user: ${userId} with ${formattedQuiz.length} questions`);
...
export const getQuizScore = async (quizOwnerId) => {
  try {
    console.log(`📊 Getting quiz score for quiz owner: ${quizOwnerId}`);
    ...
    if (matchError || !match) {
      console.log(`📭 No match found between current user and quiz owner: ${quizOwnerId}`);
      return null;
    }
    ...
    console.log(`✅ Retrieved quiz score: ${score}% for quiz owner: ${quizOwnerId}`);
    return score;
  } catch (error) {
    console.error('❌ Error in getQuizScore:', error);
    return null;
  }
};
...
export const getAnalyticsData = async (days = 7) => {
  try {
    ...
    // Get current user's likes sent with timestamps (only active likes)
    ...
    // Get current user's likes received with timestamps (only active likes)
    ...
    // Get current user's matches with timestamps (only active matches)
...
export const getAnalyticsSummary = async () => {
  try {
    ...
    // Get likes received (likes where current user is the receiver)
    const { data: likesReceived, error: likesReceivedError } = await supabase
      .from('like')
      .select('id')
      .eq('receiver_id', currentUser.id)
    ...
    // Get likes given (likes where current user is the sender)
...
// Function to get all quiz scores for the current user
export const getAllQuizScores = async () => {
  try {
    console.log(`📊 Getting all quiz scores for current user`);
    ...
    const { data: userMatches, error: matchesError } = await supabase
      .from('match')
      .select('id, user_1_id, user_2_id, user_1_score, user_2_score')
      .or(`user_1_id.eq.${currentUser.id},user_2_id.eq.${currentUser.id}`);
    ...
    // Filter matches that have quiz scores
    const matchesWithScores = userMatches.filter(match => {
      if (match.user_1_id === currentUser.id) {
        return match.user_1_score !== null && match.user_1_score !== undefined;
      } else {
        return match.user_2_score !== null && match.user_2_score !== undefined;
      }
    });

    // Get user profiles for all matched users to get their names
    const matchedUserIds = matchesWithScores.map(match => 
      match.user_1_id === currentUser.id ? match.user_2_id : match.user_1_id
    );
    ...
    const quizScores = matchesWithScores.map(match => {
      const otherUserId = match.user_1_id === currentUser.id ? match.user_2_id : match.user_1_id;
      const score = match.user_1_id === currentUser.id ? match.user_1_score : match.user_2_score;
      
      return {
        quizOwnerId: otherUserId,
        quizOwnerName: userNames[otherUserId] || 'Unknown User',
        score: score
      };
    });
    ...
    console.error('❌ Error in getAllQuizScores:', error);
    throw error;
  }
};
