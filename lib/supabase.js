    ...
    const processedProfiles = await Promise.all(
      filteredProfiles.map(async (profile) => {
        ...
        if (currentUserGeolocation && profile.geolocation) {
          ...
          console.log(`📍 No geolocation data for ${profile.name}, including in results`);
        }

        // Calculate distance if both users have residences (for now, just show if they're in the same residence)
