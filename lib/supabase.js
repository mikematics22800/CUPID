...
export async function signInWithEmail(email, password, setLoading) {
  try {
    ...
    // Update geolocation on login
    try {
      console.log('📍 Updating geolocation on login');
...
export async function register(firstName, lastName, phone, email, sex, birthday, password, onBack, setLoading, onRegistrationSuccess) {
  try {
    ...
    try {
      console.log('📍 Getting current location for registration');
      const location = await getCurrentLocationAndresidence();
      if (location) {
        geolocation = location.geolocation;
        console.log('✅ Location obtained for registration:', geolocation);
      }
    } catch (locationError) {
      console.error('❌ Error getting location for registration:', locationError);
      // Continue with registration even if location fails
    }
...
// Function to get users within x miles of your residence
export const getUsersWithinDistance = async (maxDistance = 50, limit = 50) => {
  try {
    console.log(`🔍 Finding users within ${maxDistance} miles of your residence`);
    ...
    if (!currentUserResidence && !currentUserGeolocation) {
    ...
    const { data: allProfiles, error: profilesError } = await supabase
      ...
      .not('residence', 'is', null) // Only users with residence set
    ...
    console.log(`📊 Found ${allProfiles.length} users with residence data`);
    ...
    const processedProfiles = await Promise.all(
      allProfiles.map(async (profile) => {
        ...
        // Method 1: Use geolocation coordinates if available for both users
        if (currentUserGeolocation && profile.geolocation) {
          try {
            const currentUserCoords = Array.isArray(currentUserGeolocation) ? currentUserGeolocation : JSON.parse(currentUserGeolocation);
            const profileCoords = Array.isArray(profile.geolocation) ? profile.geolocation : JSON.parse(profile.geolocation);
            ...
            if (currentUserCoords.length === 2 && profileCoords.length === 2) {
              ...
              console.log(`📍 Coordinate-based distance to ${profile.name}: ${distance} miles`);
            }
          } catch (distanceError) {
            console.error('❌ Error calculating coordinate distance:', distanceError);
          }
        }
        
        // Method 2: Use residence addresses if coordinate method failed or unavailable
        if (distance === null && currentUserResidence && profile.residence) {
          try {
            ...
            if (distance !== null) {
              distanceMethod = 'address';
              console.log(`📍 Address-based distance to ${profile.name}: ${distance} miles`);
            } else {
              // Check if they're in the same residence
              if (currentUserResidence === profile.residence) {
                distance = 0;
        ...
        return {
          ...
          withinDistance: distance !== null && distance <= maxDistance
        };
      })
    );
    ...
    console.log(`📍 Distance filtering: ${maxDistance} miles max`);
